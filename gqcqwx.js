...
if (typeof $.ReadData('gqcqwxcookie') !== "undefined"){
    
    $.GrabData('token','newtoken',"","广汽传祺小程序 [ 新值 ]",0);
    
    let Old = $.ReadData('gqcqwxcookie');
    let New = $.ReadData('gqcqwxcookie');

    if (New !== Old){
        $.GrabData(New,'gqcqwxcookie');
        $.n('广汽传祺小程序token值[ 已更新 ]','gqcqwxcookie',New);
    }else {
        $.n('广汽传祺小程序token值[ 无需更新 ]','gqcqwxcookie',Old);
    }

}else {
    $.GrabData('token','gqcqwxcookie',"","广汽传祺小程序",0);
}
